...
if (function_exists('add_theme_support')) {
	...
	add_image_size('large', 870, '', true); // Large Thumbnail
	...
	if (class_exists('MultiPostThumbnails')) {
		...
		$types = array('post', 'page', 'html5-blank');
...

{
    ...
    if (!is_admin()) {
		...
		wp_register_script('conditionizr', get_template_directory_uri() . '/js/conditionizr.min.js', array(), '1.0');
		...
		wp_register_script('scripts', get_template_directory_uri() . '/js/scripts.js', array(), '1.0');
...
//add_action('save_post', 'wpt_save_events_meta', 1, 2); // save the Featured Image
...

{
    ...
    register_post_type('html5-blank', // Register Custom Post Type
        ...
        'labels' => array(
            ...
            'name' => __('All Minimal Posts', 'html5blank'), // Rename these to suit



            'singular_name' => __('Minimal Posts', 'html5blank'),
            ...
            'add_new_item' => __('Add New Minimal Post', 'html5blank'),
            ...
            'edit_item' => __('Edit Minimal Post', 'html5blank'),



            'new_item' => __('New Minimal Post', 'html5blank'),



            'view' => __('View HMinimal Post', 'html5blank'),



            'view_item' => __('View Minimal Post', 'html5blank'),



            'search_items' => __('Search Minimal Post', 'html5blank'),



            'not_found' => __('No Minimal Posts found', 'html5blank'),



            'not_found_in_trash' => __('No Minimal Posts found in Trash', 'html5blank')
        ...
        'taxonomies' => array(
            ...
            'category' // Add Category and Post Tags support
...
//Allow HTML5 Posts to appear on front page with rest of the posts
...
function add_my_custom_post_type( $query ) {
	...
	if ( is_tag() )
        ...
        $query->set('post_type', array( 'html5-blank', 'post' ) );
...
function new_trim_excerpt($text) {
        ...
        if ( '' == $text ) {
                ...
                $_text = substr($text, $_x, $_y - $_x + 2);



				$text = str_replace($_text,'',$text);
		...
		if ( '' == $text ) {
			...
			$text = strip_tags($text, '<p><a><div><em><strong><i><b>');
...
add_filter('new_trim_excerpt', 'wptexturize');



add_filter('new_trim_excerpt', 'convert_smilies');



add_filter('new_trim_excerpt', 'convert_chars');



add_filter('new_trim_excerpt', 'wpautop');



add_filter('new_trim_excerpt', 'shortcode_unautop');
